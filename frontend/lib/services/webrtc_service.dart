...
class WebRTCService extends ChangeNotifier {
  ...
  Future<void> teardown() async {
    ...
    // Set this FIRST, synchronously, before any await. If teardown() is
    // called again while this call is still mid-flight (e.g. a WS error
    // handler tears down RTC, and the widget disposes moments later while
    // that's still running), the second call's guard check above must see
    // _disposed=true immediately — otherwise both calls run through to the
    // final dispose() below and the second one throws "used after being
    // disposed". Setting it at the END (after the awaits) was the bug:
    // there was a window where two concurrent calls could both pass the
    // guard.
    _disposed = true;
